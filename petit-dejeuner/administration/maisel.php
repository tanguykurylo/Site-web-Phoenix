    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-offset-1 col-md-10">
          <h1 class="centered-title">Commandes Maisel</h1>
          ...
          <div class="panel panel-default">
            ...
            <table class="table table-striped table-bordered table-hover">
              ...
              <tr>
                <td> <?php
                  $requete = $base_de_donnees->query('SELECT SUM(petit_dejeuner_anglais) as total_commandes FROM commandes WHERE localisation = \'maisel\'');
                  $total = $requete->fetch();
                ...
                <td> <?php
                  $requete = $base_de_donnees->query('SELECT SUM(petit_dejeuner_chinois) as total_commandes FROM commandes WHERE localisation = \'maisel\'');
                  $total = $requete->fetch();
                  echo $total['total_commandes'];
                  $requete->closeCursor();
                  ?>
                ...
                <td> <?php
                  $requete = $base_de_donnees->query('SELECT SUM(petit_dejeuner_int) as total_commandes FROM commandes WHERE localisation = \'maisel\'');
                  $total = $requete->fetch();
                  echo $total['total_commandes'];
                  $requete->closeCursor();
                  ?>
          ...
          <?php
            ...
            for ($heure_livraison = $heure_prochaine_livraison; $heure_livraison <= $heure_derniere_livraison; $heure_livraison += (15*60) ){
              ...
              $petit_dejeuners = $base_de_donnees->prepare('SELECT * FROM commandes WHERE heure_de_livraison = :h AND localisation = \'maisel\'');
