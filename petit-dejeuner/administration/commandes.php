    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-offset-1 col-md-10">
          <h1 class="centered-title">Commandes</h1>
          ...
          <div class="panel panel-default">
            ...
            <table class="table table-striped table-bordered table-hover">
              ...
              <tr>
                <td> <?php
                  $requete = $base_de_donnees->query('SELECT SUM(petit_dejeuner_anglais) as total_commandes FROM commandes');
                  $total = $requete->fetch();
                ...
                <td> <?php
                  $requete = $base_de_donnees->query('SELECT SUM(petit_dejeuner_chinois) as total_commandes FROM commandes');
                  $total = $requete->fetch();
                  echo $total['total_commandes'];
                  $requete->closeCursor();
                  ?>
                ...
                <td> <?php
                  $requete = $base_de_donnees->query('SELECT SUM(petit_dejeuner_int) as total_commandes FROM commandes');
                  $total = $requete->fetch();
                  echo $total['total_commandes'];
                  $requete->closeCursor();
                  ?>
          ...
          <?php
            ...
            for ($heure_livraison = $heure_premiere_livraison; $heure_livraison <= $heure_derniere_livraison; $heure_livraison += (15*60) ){

              $heure_lisible = date("H:i", ($heure_livraison + (60*60) ) ); // on compense le décalage horaire du serveur (GMT+1 pour nous)

              $petit_dejeuners = $base_de_donnees->prepare('SELECT * FROM commandes WHERE heure_de_livraison = :h');
              ...
              if ($petit_dejeuners->rowCount() != 0){
                ...
                <div class="panel panel-default">
                  ...
                  <table class="table table-striped table-bordered table-hover">
                    ...
                    ?>
                    <tr>
                      <td class="col-xs-1"><strong>Total</strong></td>
                      <td class="col-xs-1"></td>
                      <td class="col-xs-1"></td>
                      <td class="col-xs-1"><?php
                        $requete = $base_de_donnees->prepare('SELECT SUM(petit_dejeuner_anglais) as total_commandes FROM commandes WHERE heure_de_livraison = :h');
                        $requete->execute(array('h' => date('Y-m-d H:i:s', $heure_livraison)));
                        $total = $requete->fetch();
                        echo $total['total_commandes'];
                        $requete->closeCursor();
                        ?></td>
                      <td class="col-xs-1"><?php
                        $requete = $base_de_donnees->prepare('SELECT SUM(petit_dejeuner_chinois) as total_commandes FROM commandes WHERE heure_de_livraison = :h');
                        $requete->execute(array('h' => date('Y-m-d H:i:s', $heure_livraison)));
                        $total = $requete->fetch();
                        echo $total['total_commandes'];
                        $requete->closeCursor();
                        ?></td>
                      <td class="col-xs-1"><?php
                        $requete = $base_de_donnees->prepare('SELECT SUM(petit_dejeuner_int) as total_commandes FROM commandes WHERE heure_de_livraison = :h');
                        $requete->execute(array('h' => date('Y-m-d H:i:s', $heure_livraison)));
                        $total = $requete->fetch();
                        echo $total['total_commandes'];
                        $requete->closeCursor();
